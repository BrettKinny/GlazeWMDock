...
`0`), so releases bump the **3rd** field — `1.0.0`, `1.0.1`, `1.0.2`, … — which
maps to package versions `1.0.0.0`, `1.0.1.0`, `1.0.2.0`.
...
- **Multi-monitor grouping** — the strip now groups workspaces by monitor
  (ordered left-to-right by physical position). Each monitor's currently
  displayed workspace is the bold filled circled digit; every other workspace is
  a plain digit. Focus is shown by brackets: every monitor except the one you're
  on is bracketed, so the un-bracketed group is where you are (e.g.
  `❸ 5 [1 2 4 ❻]`). The Dock API shares one band across all monitors, so this
  replaces the previous behavior where every monitor's dock showed an identical,
  focus-only strip.
